...
(d3.geo.conicEqualArea = function () {
    ...
    d3.geo.prepare = function (methods) {
        return {
...
(d3.geo.mercator = function () {
    ...
    function d3_interpolateString(a, b) {
        ...
        for (i = 0, n = q.length; (m = d3_interpolate_number.exec(a)
            ) && i < n;
        ++i) {
            o = q[i];
            if (o.x == m[0]) {
    ...
    d3.prepare = function (string) {
        var g = d3_document.createElementNS(d3.ns.prefix.svg, "g");
        return (d3.prepare = function (string) {
            if (string != null) {
    ...
    function d3_interpolateTransform(a, b) {
        var s = [], q = [], n, A = d3.prepare(a), B = d3.prepare(b), ta = A.translate, tb = B.translate, ra = A.rotate, rb = B.rotate, wa = A.skew, wb = B.skew, ka = A.scale, kb = B.scale;
        ...
        wiggle: function (data) {
            ...
            for (j = 1; j < m; ++j) {
                ...
                for (i = 0, s2 = 0, dx = x[j][0] - x[j - 1][0]; i < n; ++i) {
                    for (k = 0, s3 = (data[i][j][1] - data[i][j - 1][1]) 
                        / (2 * dx);
                    k < i;
                    ++k) {
        ...
        function tree(d, i) {
            ...
            function apportion(node, previousSibling, ancestor) {
                if (previousSibling) {
                    ...
                    while (vim = d3_layout_treeRight(vim), vip = d3_layout_treeLeft(vip)
                        , vim && vip) {
    ...
    d3_transitionPrototype.attr = function (nameNS, value) {
        ...
        var interpolate = nameNS == "prepare" ? d3_interpolateTransform : d3_interpolate, name = d3.ns.qualify(nameNS);
    ...
    function d3_svg_axisX(selection, x) {
        selection.attr("prepare", function (d) {
    ...
    d3.svg.brush = function () {
        ...
        function redraw(g) {
            g.selectAll(".resize").attr("prepare", function (d) {
